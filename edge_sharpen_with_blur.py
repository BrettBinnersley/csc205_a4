...
if __name__ == '__main__':
    ...
    output_filename = oname + '_sharpen_and_blurred.png'

    # Read input and generate guassian blur matrix
    image_intensities = read_image_greyscale(input_filename)
    print "Sharpening Edges with weight: 15 & blur stdDev of 1 "

    # Apply the matrix blur
    blur_matrix = GenGaussianBlur5x5(1)
    blurred_image = ApplyBlur(image_intensities, blur_matrix)
    hl = GenHL()
    sharpend_image = ApplyHL(blurred_image, hl, 15.0)

    # Plot output
    write_png_grayscale(output_filename, sharpend_image)  # F(x) histogram
